    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (args.length == 0){
                ...
                if (p.hasPermission(Main.getPermissionNotice("Permissions.Everything")) || p
                    ...
                    if (SystemManager.existsPlayerData(SystemManager.getUUIDByName(args[0]))) {
                        ...
                        w.setText(Main.getPrefix() + Main.getMSG("Messages.Warn-System.Warns.Player-Info")
                                ...
                                .replace("%warns%", "" + BanManager.getWarns(SystemManager.getUUIDByName(target))));
                        ...
                        tc1.setText(
                                Main.getPrefix() + Main.getMSG("Messages.Warn-System.Warns.Action-Mute-Button"));
                ...
                if (p.hasPermission(Main.getPermissionNotice("Permissions.Everything")) || p
                        .hasPermission(Main.getPermissionNotice("Permissions.Warns.Clear"))) {
                    if (SystemManager.existsPlayerData(SystemManager.getUUIDByName(args[0]))) {

                        BanManager.clearWans(SystemManager.getUUIDByName(args[0]));

                        //PREFIX + "You Cleared the "
                        p.sendMessage(Main.getPrefix() + Main.getMSG("Messages.Warn-System.Warn.Clear")
                    ...
                    p.sendMessage(Main.getPrefix() + Main.getMSG("Messages.System.No-Permission")
                            .replace("%permission%", Main.getPermissionNotice("Permissions.Warns.Clear")));
                }
            } else {
            ...
            if (args.length == 1) {
                ...
                if (SystemManager.existsPlayerData(SystemManager.getUUIDByName(args[0]))) {
                    ...
                    sender.sendMessage(
                            Main.getPrefix() + Main.getMSG("Messages.Warn-System.Warns.Player-Info")
