    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (p.hasPermission(Main.getPermissionNotice("Permissions.Everything")) || p
                    .hasPermission(Main.getPermissionNotice("Permissions.BanHistory.See"))) {
                if (MySQL.isConnected()) {
                    if (args.length == 1) {
                        if (SystemManager.existsPlayerData(SystemManager.getUUIDByName(args[0]))) {
                            new BukkitRunnable() {
                                ...
                                public void run() {
                                    ResultSet rs = MySQL.getResult(
                                            "SELECT * FROM ReportSystem_banhistory WHERE BANNED_UUID = '" + SystemManager
                                                    .getUUIDByName(args[0]) + "'");
                                    try {
                                        if (rs.next()) {
                                            p.sendMessage("");
                                            p.sendMessage(
                                                    Main.getPrefix() + Main.getMSG("Messages.Ban-System.History.Title")
                                                            + args[0]);
                                            p.sendMessage("");
                                            if (rs.getFetchSize() > 10) {
                                                p.sendMessage(Main.getPrefix() + Main
                                                        .getMSG("Messages.Ban-System.History.History-too-long"));
                                            }
                                            rs.setFetchSize(10);
                                            while (rs.next()) {
                                                p.sendMessage(Main.getPrefix() + Main
                                                        .getMSG("Messages.Ban-System.History.Prefix.BanID") + rs.getInt("id"));
                                                p.sendMessage(Main.getPrefix() + Main
                                                        .getMSG("Messages.Ban-System.History.Prefix.Reason") + rs
                                                        .getString("REASON"));
                                                p.sendMessage(Main.getPrefix() + Main
                                                        .getMSG("Messages.Ban-System.History.Prefix.BannedBy") + SystemManager
                                                        .getUsernameByUUID(rs.getString("TEAM_UUID")));
                                                p.sendMessage(Main.getPrefix() + Main
                                                        .getMSG("Messages.Ban-System.History.Prefix.BannedSince") + new Date(
                                            ...
                                            p.sendMessage(Main.getPrefix() + Main
                                                    .getMSG("Messages.Ban-System.History.No-History-Available"));
                                        }
                                        p.sendMessage("");
                            ...
                            p.sendMessage(Main.getPrefix() + Main
                                    .getMSG("Messages.Ban-System.History.Target-Never-Joined"));
                        }
                        ...
                        if (SystemManager.existsPlayerData(SystemManager.getUUIDByName(args[0]))) {
                            if (args[1].equalsIgnoreCase("clear")) {
                                new BukkitRunnable() {
                                    ...
                                    public void run() {
                                        MySQL.update(
                                                "DELETE FROM ReportSystem_banhistory WHERE BANNED_UUID = '" + SystemManager
                                                        .getUUIDByName(args[0]) + "'");
                                    }

                                }.runTaskAsynchronously(Main.getInstance());
                                p.sendMessage(
                                        Main.getPrefix() + Main.getMSG("Messages.Ban-System.History.Cleared"));
                            }
                        } else {
                            p.sendMessage(Main.getPrefix() + Main
                                    .getMSG("Messages.Ban-System.History.Target-Never-Joined"));
                ...
                p.sendMessage(Main.getPrefix() + Main.getMSG("Messages.System.No-Permission")
                        .replace("%permission%", Main.getPermissionNotice("Permissions.BanHistory.See")));
            }
        } else {
            if (MySQL.isConnected()) {
                if (args.length == 1) {
                    if (SystemManager.existsPlayerData(SystemManager.getUUIDByName(args[0]))) {
                            ...
                            public void run() {
                                ...
                                try {
                                    if (rs.next()) {
                                        ...
                                        sender.sendMessage("");
                                        if (rs.getFetchSize() > 10) {
                                            sender.sendMessage(Main.getPrefix() + Main
                                                    .getMSG("Messages.Ban-System.History.History-too-long"));
                                        }
                                        rs.setFetchSize(10);
                                        while (rs.next()) {
                                            sender.sendMessage(
                                                    Main.getPrefix() + Main.getMSG("Messages.Ban-System.History.Prefix.BanID")
                                                            + rs.getInt("id"));
                                            sender.sendMessage(Main.getPrefix() + Main
                                                    .getMSG("Messages.Ban-System.History.Prefix.Reason") + rs
                                                    .getString("REASON"));
                        ...
                        sender.sendMessage(
                                Main.getPrefix() + Main.getMSG("Messages.Ban-System.History.Target-Never-Joined"));
                    }
                } else if (args.length == 2) {
                    if (SystemManager.existsPlayerData(SystemManager.getUUIDByName(args[0]))) {
                        if (args[1].equalsIgnoreCase("clear")) {
            ...
            if (BanManager.isBanned(uuid)) {
                if (BanManager.getBanReason(uuid).matches(reason)) {
                    if (BanManager.getBanEnd(uuid) == -1) {
                        return ChatColor.DARK_GRAY + "(" + Main.getMSG("Layout.Ban.Length-Values.Permanently")
                                + ChatColor.DARK_GRAY + ")";
                    } else {
