        ...
        if (sender instanceof Player) {
            ...
            if (MySQL.isConnected()) {
                if (p.hasPermission(Main.getPermissionNotice("Permissions.Everything")) || p
                        .hasPermission(Main.getPermissionNotice("Permissions.UnMute.Use"))) {
                    if (args.length == 1) {
                        if (SystemManager.getUUIDByName(args[0]) != null) {
                            if (BanManager.isMuted(SystemManager.getUUIDByName(args[0]))) {
                                if (BanManager.unmute(SystemManager.getUUIDByName(args[0]))) {
                                    ReportManager
                                            .sendNotify("UNMUTE", p.getName(), SystemManager.getUUIDByName(args[0]),
                                                    null);
                                    p.sendMessage(
                                            Main.getPrefix() + Main.getMSG("Messages.Mute-System.UnMute.Success"));
                                } else {
                                    p.sendMessage(
                                            Main.getPrefix() + Main.getMSG("Messages.Mute-System.UnMute.Error"));
                                }
                            } else {
                                p.sendMessage(
                                        Main.getPrefix() + Main.getMSG("Messages.Mute-System.UnMute.Not-Muted"));
                            }
                        } else {
                            p.sendMessage(
                ...
                if (args.length == 1) {
                    if (SystemManager.getUUIDByName(args[0]) != null) {
                        if (BanManager.isMuted(SystemManager.getUUIDByName(args[0]))) {
                            if (BanManager.unmute(SystemManager.getUUIDByName(args[0]))) {
                                ReportManager
                                        .sendNotify("UNMUTE", "Console", SystemManager.getUUIDByName(args[0]), null);
                                sender.sendMessage(
                                        Main.getPrefix() + Main.getMSG("Messages.Mute-System.UnMute.Success"));
                            } else {
                                sender.sendMessage(
