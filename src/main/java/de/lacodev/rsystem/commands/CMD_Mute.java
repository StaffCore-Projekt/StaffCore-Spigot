        ...
        if (sender instanceof Player) {
            ...
            if (MySQL.isConnected()) {
                if (p.hasPermission(Main.getPermissionNotice("Permissions.Everything")) || p
                        .hasPermission(Main.getPermissionNotice("Permissions.Mute.Use"))) {
                    if (args.length == 2) {
                        int id;
                        if (SystemManager.getUUIDByName(args[0]) != null) {
                                    ...
                                    if (!args[1].matches(p.getName())) {
                                        if (Main.getInstance().getConfig()
                                            ...
                                            if (p.hasPermission(Main.getInstance().getConfig()
                                                    ...
                                                    + args[1])) {
                                                if (BanManager.submitMute(SystemManager.getUUIDByName(args[0]),
                                                        BanManager.getMuteReasonFromID(id), p.getUniqueId().toString())) {
                                                    if (Main.getInstance().actionbar) {
                                                        ActionBarAPI.sendActionBar(p,
                            ...
                            if (Main.getInstance().actionbar) {
                                ActionBarAPI.sendActionBar(p,
                                        Main.getPrefix() + Main.getMSG("Messages.Mute-System.Cannot-find-player"));
                            } else {
                                p.sendMessage(
                                        Main.getPrefix() + Main.getMSG("Messages.Mute-System.Cannot-find-player"));
                            }
                ...
                if (args.length == 2) {
                    ...
                    if (SystemManager.getUUIDByName(args[0]) != null) {
                        if (BanManager.existsMuteID(id)) {
                            if (!BanManager.isMuted(SystemManager.getUUIDByName(args[0]))) {
                                if (BanManager.submitMute(SystemManager.getUUIDByName(args[0]),
                                        BanManager.getMuteReasonFromID(id), "Console")) {
                                    sender
                                            .sendMessage(Main.getPrefix() + Main.getMSG("Messages.Mute-System.Created"));
                                } else {
                            ...
                            if (reasons.size() > 0) {
                                ...
                                sender
                                        .sendMessage(Main.getPrefix() + Main.getMSG("Messages.Mute-System.No-Reasons"));
                            }
                            sender.sendMessage("");
                        }
                    } else {
                        ...
                        for (int i = 0; i < reasons.size(); i++) {
                            sender.sendMessage(
                                    Main.getPrefix() + ChatColor.DARK_GRAY + "- " + ChatColor.YELLOW + "" + reasons
                                            ...
                                            + BanManager.getBanLength(reasons.get(i).getLength()) + ChatColor.DARK_GRAY
                                            + ")");
                        }
                    } else {
