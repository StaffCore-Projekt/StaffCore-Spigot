    ...
    if (sender instanceof Player) {
      ...
      if (MySQL.isConnected()) {
        if (p.hasPermission(Main.getPermissionNotice("Permissions.Everything")) || p
            .hasPermission(Main.getPermissionNotice("Permissions.Ban.Use"))) {
          if (args.length == 2) {
            int id;
            if (SystemManager.getUUIDByName(args[0]) != null) {
              try {
                ...
                if (Main.getInstance().actionbar) {
                  ActionBarAPI.sendActionBar(p,
                      Main.getPrefix() + Main.getMSG("Messages.Ban-System.NotValidID"));
                } else {
                  ...
                  if (!args[0].matches(p.getName())) {
                    if (Main.getInstance().getConfig()
                      ...
                      if (p.hasPermission(Main.getInstance().getConfig()
                          ...
                          + args[1])) {
                        BanManager.submitBan(SystemManager.getUUIDByName(args[0]),
                            BanManager.getBanReasonFromID(id), p.getUniqueId().toString());
                        if (Main.getInstance().actionbar) {
                          ActionBarAPI.sendActionBar(p,
                        ...
                        p.sendMessage(
                            Main.getPrefix() + Main.getMSG("Messages.System.No-Permission")
                                .replace("%permission%", Main.getInstance().getConfig().getString(
              ...
              if (Main.getInstance().actionbar) {
                ActionBarAPI.sendActionBar(p,
                    Main.getPrefix() + Main.getMSG("Messages.Ban-System.Cannot-find-player"));
              } else {
                p.sendMessage(
                    Main.getPrefix() + Main.getMSG("Messages.Ban-System.Cannot-find-player"));
              }
            ...
            if (args.length == 1) {
              ...
              if (Main.getInstance().actionbar) {
                ActionBarAPI
                    .sendActionBar(p, Main.getPrefix() + Main.getMSG("Messages.Ban-System.Usage"));
              } else {
                ...
                for (int i = 0; i < reasons.size(); i++) {
                  p.sendMessage(
                      Main.getPrefix() + ChatColor.DARK_GRAY + "- " + ChatColor.YELLOW + reasons
                          ...
                          + "" + BanManager.getBanLength(reasons.get(i).getLength())
                          + ChatColor.DARK_GRAY + ")");
                }
              } else {
                p.sendMessage(Main.getPrefix() + Main.getMSG("Messages.Ban-System.No-Reasons"));
        ...
        if (args.length == 2) {
          ...
          if (SystemManager.getUUIDByName(args[0]) != null) {
            ...
            if (BanManager.existsBanID(id)) {
              if (!BanManager.isBanned(SystemManager.getUUIDByName(args[0]))) {
                BanManager.submitBan(SystemManager.getUUIDByName(args[0]),
                    BanManager.getBanReasonFromID(id), "Console");
              ...
              if (reasons.size() > 0) {
                ...
                sender
                    .sendMessage(Main.getPrefix() + Main.getMSG("Messages.Ban-System.No-Reasons"));
              }
              sender.sendMessage("");
            }
          } else {
          ...
          if (reasons.size() > 0) {
            for (int i = 0; i < reasons.size(); i++) {
              sender.sendMessage(
                  Main.getPrefix() + ChatColor.DARK_GRAY + "- " + ChatColor.YELLOW + reasons.get(i)
                      .getName() + ChatColor.DARK_GRAY + " (" + ChatColor.YELLOW + reasons.get(i)
                      .getID() + ChatColor.DARK_GRAY + " | " + ChatColor.YELLOW + "" + BanManager
                      .getBanLength(reasons.get(i).getLength()) + ChatColor.DARK_GRAY + ")");
            }
          } else {
