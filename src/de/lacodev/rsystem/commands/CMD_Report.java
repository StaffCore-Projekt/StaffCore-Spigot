        ...
        if (args.length == 0) {
          ...
          if (args[0].toLowerCase().equalsIgnoreCase("templates")) {
            ...
            if (reasons.size() > 0) {
              ...
              p.sendMessage(
                  Main.getPrefix() + Main.getMSG("Messages.Report-System.No-Reportreasons"));
            }
            p.sendMessage("");
          } else {
          ...
          if (args[0].toLowerCase().equalsIgnoreCase("claim")) {
            ...
            if (p.hasPermission(Main.getPermissionNotice("Permissions.Report.Claim")) || p
              ...
              p.sendMessage(Main.getPrefix() + Main.getMSG("Messages.System.No-Permission")
                  .replace("%permission%", Main.getPermissionNotice("Permissions.Report.Claim")));
            }
              ...
              if (target != null) {
                if (target != p) {
                  ...
                  if (!p.hasPermission(Main.getPermissionNotice("Permissions.Everything")) || !p
                      .hasPermission(Main.getPermissionNotice("Permissions.Report.Spam.Bypass"))) {
                    Listener_Chat.spam.put(p, System.currentTimeMillis()
                        + Main.getInstance().getConfig().getInt("Report-Spam.Duration-in-Seconds")
                        * 1000);
                  }
                } else {
                  p.sendMessage(
