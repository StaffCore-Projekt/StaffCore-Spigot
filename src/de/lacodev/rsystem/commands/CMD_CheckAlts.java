...
public class CMD_CheckAlts implements CommandExecutor {
  ...
  public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
    if (sender instanceof Player) {
      ...
      if (p.hasPermission(Main.getPermissionNotice("Permissions.Everything")) || p
          .hasPermission(Main.getPermissionNotice("Permissions.CheckAlts.Use"))) {
        ...
        if (args.length == 1) {
          ...
          if (SystemManager.existsPlayerData(SystemManager.getUUIDByName(target))) {
            ...
            p.sendMessage(
                Main.getPrefix() + Main.getMSG("Messages.Altaccount-Check.Collecting-Data")
                    .replace("%ip%",
                        SystemManager.getLastKnownIP(SystemManager.getUUIDByName(target))));
                ...
                try {
                  if (rs != null) {
                    p.sendMessage(Main.getPrefix() + Main
                        .getMSG("Messages.Altaccount-Check.Possible-Alts-Title")
                        .replace("%target%", target));
                    while (rs.next()) {
                      if (!rs.getString("PLAYERNAME").equalsIgnoreCase(target)) {
                        p.sendMessage("");
                        p.sendMessage(Main.getPrefix() + Main
                            .getMSG("Messages.Altaccount-Check.Prefix.Username") + rs
                            .getString("PLAYERNAME"));
                        TextComponent tc = new TextComponent();
                        tc.setText(Main.getPrefix() + Main
                            .getMSG("Messages.Altaccount-Check.More-Info-Button"));

                        String listString =
                            Main.getMSG("Messages.Altaccount-Check.Prefix.Last-Online")
                                + SystemManager.getLastOnline(rs.getString("UUID")) + "\n" + Main
                                .getMSG("Messages.Altaccount-Check.Prefix.Reports") + ReportManager
                                .getReports(rs.getString("UUID"))
                                + "\n" + Main.getMSG("Messages.Altaccount-Check.Prefix.Bans")
                                + BanManager.getBans(rs.getString("UUID")) + "\n" + Main
                                .getMSG("Messages.Altaccount-Check.Prefix.Mutes") + BanManager
                                .getMutes(rs.getString("UUID")) + "\n" + Main
                                .getMSG("Messages.Altaccount-Check.Prefix.Warns") + BanManager
                                .getWarns(rs.getString("UUID"))
                                + "\n \n" + Main.getMSG("Messages.Altaccount-Check.Prefix.Banned")
                                + getBanningState(rs.getString("UUID")) + "\n" + Main
                                .getMSG("Messages.Altaccount-Check.Prefix.Muted") + getMutingState(
                                rs.getString("UUID"));
                    ...
                    p.sendMessage(
                        Main.getPrefix() + Main.getMSG("Messages.Altaccount-Check.Nothing-found"));
                  }
            ...
            p.sendMessage(
                Main.getPrefix() + Main.getMSG("Messages.Altaccount-Check.Player-Not-Found"));
          }
  ...
  private String getMutingState(String uuidByName) {
    if (BanManager.isMuted(uuidByName)) {
      return Main.getMSG("Messages.Altaccount-Check.State.Muted") + BanManager
  ...
  private String getBanningState(String uuidByName) {
    if (BanManager.isBanned(uuidByName)) {
      return Main.getMSG("Messages.Altaccount-Check.State.Banned") + BanManager
