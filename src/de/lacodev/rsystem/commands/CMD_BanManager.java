      ...
      if (MySQL.isConnected()) {
        if (args.length >= 4) {
          if (args[0].toLowerCase().equalsIgnoreCase("addreason")) {
            if (p.hasPermission(Main.getPermissionNotice("Permissions.BanManager.addreason")) || p
              ...
              if (timeunit.toLowerCase().matches("d") || timeunit.toLowerCase().matches("h")
                  || timeunit.toLowerCase().matches("m") || args[2].toLowerCase()
                ...
                if (args[1].toLowerCase().matches("ban")) {
                  if (args[2].toLowerCase().equalsIgnoreCase("perma")) {
                    if (!BanManager.existsBanReason(reason)) {
                      ...
                      if (Main.getInstance().actionbar) {
                        ActionBarAPI.sendActionBar(p, Main.getPrefix() + Main
                            .getMSG("Messages.BanManager.Add-Reason.Success"));
                      } else {
                      ...
                      if (Main.getInstance().actionbar) {
                        ActionBarAPI.sendActionBar(p, Main.getPrefix() + Main
                            .getMSG("Messages.BanManager.Add-Reason.Already-Existing"));
                      } else {
                        p.sendMessage(Main.getPrefix() + Main
                            .getMSG("Messages.BanManager.Add-Reason.Already-Existing"));
                      }
                    ...
                    if (!BanManager.existsBanReason(reason)) {
                      BanManager.createBanReason(reason, timeunit,
                  ...
                  if (!BanManager.existsMuteReason(reason)) {
                    BanManager.createMuteReason(reason, timeunit,
                        Integer.valueOf(args[2].substring(0, args[2].length() - 1)));
                    if (Main.getInstance().actionbar) {
                      ActionBarAPI.sendActionBar(p,
                          Main.getPrefix() + Main.getMSG("Messages.BanManager.Add-Reason.Success"));
                    } else {
                      p.sendMessage(
                          Main.getPrefix() + Main.getMSG("Messages.BanManager.Add-Reason.Success"));
                    }
                  ...
                  if (Main.getInstance().actionbar) {
                    ActionBarAPI.sendActionBar(p, Main.getPrefix() + Main
                        .getMSG("Messages.BanManager.Add-Reason.Invalid-ReasonType"));
                  } else {
                    p.sendMessage(Main.getPrefix() + Main
                        .getMSG("Messages.BanManager.Add-Reason.Invalid-ReasonType"));
                  }
                ...
                if (Main.getInstance().actionbar) {
                  ActionBarAPI.sendActionBar(p, Main.getPrefix() + Main
                      .getMSG("Messages.BanManager.Add-Reason.Invalid-TimeUnit"));
                } else {
                  p.sendMessage(Main.getPrefix() + Main
                      .getMSG("Messages.BanManager.Add-Reason.Invalid-TimeUnit"));
                }
              ...
              p.sendMessage(Main.getPrefix() + Main.getMSG("Messages.System.No-Permission")
                  .replace("%permission%",
                      Main.getPermissionNotice("Permissions.BanManager.addreason")));
            }
          } else {
            p.sendMessage("");
            p.sendMessage(
                Main.getPrefix() + ChatColor.DARK_GRAY + "[" + ChatColor.YELLOW + "BanManager"
                    + ChatColor.DARK_GRAY + "]");
            p.sendMessage("");
            p.sendMessage(Main.getPrefix() + ChatColor.GRAY
                + "/banmanager addreason <Ban/Mute> <Length: 30d / Perma> <Reason>");
            p.sendMessage(
                Main.getPrefix() + ChatColor.GRAY + "/banmanager removereason <Ban/Mute> <ID>");
            p.sendMessage("");
          }
        } else if (args.length == 3) {
          if (args[0].toLowerCase().equalsIgnoreCase("removereason")) {
            int id;
            if (p.hasPermission(Main.getPermissionNotice("Permissions.BanManager.removereason"))
              ...
              if (args[1].toLowerCase().matches("ban")) {
                ...
                if (BanManager.existsBanReason(reason)) {
                  ...
                  if (Main.getInstance().actionbar) {
                    ActionBarAPI.sendActionBar(p, Main.getPrefix() + Main
                        .getMSG("Messages.BanManager.Remove-Reason.Success"));
                  } else {
                    p.sendMessage(Main.getPrefix() + Main
                        .getMSG("Messages.BanManager.Remove-Reason.Success"));
                  }
                  ...
                  if (Main.getInstance().actionbar) {
                    ActionBarAPI.sendActionBar(p, Main.getPrefix() + Main
                        .getMSG("Messages.BanManager.Remove-Reason.Not-Exists"));
                  } else {
                    p.sendMessage(Main.getPrefix() + Main
                        .getMSG("Messages.BanManager.Remove-Reason.Not-Exists"));
                  }
              ...
              p.sendMessage(Main.getPrefix() + Main.getMSG("Messages.System.No-Permission")
                  .replace("%permission%",
                      Main.getPermissionNotice("Permissions.BanManager.removereason")));
            }
          } else {
            p.sendMessage("");
            p.sendMessage(
