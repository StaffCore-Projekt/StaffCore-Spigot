						...
						if(SystemManager.getUUIDByName(args[0]) != null) {
							if(BanManager.isBanned(SystemManager.getUUIDByName(args[0]))) {
								if(BanManager.unban(SystemManager.getUUIDByName(args[0]))) {
									...
									p.sendMessage(Main.getPrefix() + Main.getMSG("Messages.Ban-System.UnBan.Success"));
								} else {
									p.sendMessage(Main.getPrefix() + Main.getMSG("Messages.Ban-System.UnBan.Error"));
								}
							} else {
								p.sendMessage(Main.getPrefix() + Main.getMSG("Messages.Ban-System.UnBan.Not-Banned"));
							}
						} else {
							p.sendMessage(Main.getPrefix() + Main.getMSG("Messages.Ban-System.UnBan.Cannot-find-player"));
						}
					} else {
						p.sendMessage(Main.getPrefix() + Main.getMSG("Messages.Ban-System.UnBan.Usage"));
					}
				} else {
	                p.sendMessage(Main.getPrefix() + Main.getMSG("Messages.System.No-Permission").replace("%permission%", Main.getPermissionNotice("Permissions.UnBan.Use")));
	            }
			} else {
					...
					if(SystemManager.getUUIDByName(args[0]) != null) {
						if(BanManager.isBanned(SystemManager.getUUIDByName(args[0]))) {
							if(BanManager.unban(SystemManager.getUUIDByName(args[0]))) {
								...
								sender.sendMessage(Main.getPrefix() + Main.getMSG("Messages.Ban-System.UnBan.Success"));
							} else {
								sender.sendMessage(Main.getPrefix() + Main.getMSG("Messages.Ban-System.UnBan.Error"));
							}
						} else {
							sender.sendMessage(Main.getPrefix() + Main.getMSG("Messages.Ban-System.UnBan.Not-Banned"));
						}
					} else {
						sender.sendMessage(Main.getPrefix() + Main.getMSG("Messages.Ban-System.UnBan.Cannot-find-player"));
					}
				} else {
