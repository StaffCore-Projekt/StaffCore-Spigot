...
public class CMD_StaffChat implements CommandExecutor {
  ...
  public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
    if (sender instanceof Player) {
      ...
      if (p.hasPermission(Main.getPermissionNotice("Permissions.StaffChat.Write")) || p
        ...
        for (Player all : Bukkit.getOnlinePlayers()) {
          if (all.hasPermission(Main.getPermissionNotice("Permissions.StaffChat.Read")) || all
              .hasPermission(Main.getPermissionNotice("Permissions.Everything"))) {
            all.sendMessage(
        ...
        p.sendMessage(Main.getPrefix() + Main.getMSG("Messages.System.No-Permission")
            .replace("%permission%", Main.getPermissionNotice("Permissions.StaffChat.Write")));
      }
    }
    return false;
  }
