  ...
  public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
    if (sender instanceof Player) {
      ...
      if (p.hasPermission(Main.getPermissionNotice("Permissions.Everything")) || p
          .hasPermission(Main.getPermissionNotice("Permissions.IpBan.Use"))) {
        if (args.length == 1) {
          ...
          if (target != null) {
            if (!BanManager.isIPBanned(target.getAddress().getAddress().toString())) {
              ...
              p.sendMessage(
                  Main.getPrefix() + Main.getMSG("Messages.Ban-System.IP-Ban.Already-banned")
                      .replace("%target%", target.getName()));
            }
          } else {
            p.sendMessage(
                Main.getPrefix() + Main.getMSG("Messages.Ban-System.IP-Ban.Target-Offline"));
          }
        } else {
