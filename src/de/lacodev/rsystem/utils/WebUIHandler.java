...
public class WebUIHandler {
	...
	public static void setupStaffCoreUI(Player player) {
		...
		MySQL.update("CREATE TABLE IF NOT EXISTS staffcoreui_notifications(id INT(6) AUTO_INCREMENT UNIQUE, USER_ID INT(6), "
				+ "NOTIFY_TEXT VARCHAR(255), NOTIFY_LINK TEXT, SEEN BOOLEAN)");
		
		// StaffCoreUI Ranks
		MySQL.update("CREATE TABLE IF NOT EXISTS staffcoreui_ranks(id INT(6) AUTO_INCREMENT UNIQUE, RANK_NAME VARCHAR(255), "
				+ "RANK_VALUE INT(6), IS_STANDARD BOOLEAN)");
		
		// StaffCoreUI Ranks Permissions
		MySQL.update("CREATE TABLE IF NOT EXISTS staffcoreui_ranks_permissions(id INT(6) AUTO_INCREMENT UNIQUE, PERMISSION VARCHAR(255), "
				+ "MIN_RANK_VALUE INT(6))");
		
		// StaffCoreUI Tasks Table
		MySQL.update("CREATE TABLE IF NOT EXISTS staffcoreui_tasks(id INT(6) AUTO_INCREMENT UNIQUE, RECIPIENT INT(6), "
				+ "TASK_ID VARCHAR(255) UNIQUE, ASSIGNED_BY VARCHAR(255), STATUS INT(6), last_update VARCHAR(255))");
		
		// StaffCoreUI Task Comments Table
		MySQL.update("CREATE TABLE IF NOT EXISTS staffcoreui_tasks_comments(id INT(6) AUTO_INCREMENT UNIQUE, TASK_ID INT(6), "
				+ "COMMENT TEXT, AUTHOR VARCHAR(255), POSTED VARCHAR(255))");
		
		// StaffCoreUI Appeals Table
		MySQL.update("CREATE TABLE IF NOT EXISTS staffcoreui_appeals(id INT(6) AUTO_INCREMENT UNIQUE, BAN_ID INT(11), UUID VARCHAR(255), "
		...
		MySQL.update("CREATE TABLE IF NOT EXISTS staffcoreui_appeals_comments(id INT(6) AUTO_INCREMENT UNIQUE, APPEAL_ID INT(6), "
				+ "COMMENT TEXT, AUTHOR VARCHAR(255), POSTED VARCHAR(255))");
		
		// StaffCoreUI Staff-Activities Table
		MySQL.update("CREATE TABLE IF NOT EXISTS staffcoreui_staff_activities(id INT(6) AUTO_INCREMENT UNIQUE, USER_ID INT(6), "
				+ "ACTIVITY_TEXT VARCHAR(500), PERFORMED VARCHAR(255))");
		
		// StaffCoreUI Settings Table
		MySQL.update("CREATE TABLE IF NOT EXISTS staffcoreui_settings(id INT(6) AUTO_INCREMENT UNIQUE, TYPE VARCHAR(255), "
				+ "VALUE TEXT)");
