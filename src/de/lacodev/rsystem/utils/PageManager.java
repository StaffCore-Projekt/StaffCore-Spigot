...
public class PageManager {
    ...
    public static List<BanReasons> getPageItems2(List<BanReasons> items, int page, int spaces) {
        ...
        for (int i = lowerBound; i < upperBound; i++) {
            try {
                reasons.add(new BanReasons(items.get(i).getName(), items.get(i).getID(), BanManager.getRawBanLength(items.get(i).getName())));
            } catch(IndexOutOfBoundsException e) {
                continue;
            }
        }
    ...
    public static List<MuteReasons> getPageItems3(List<MuteReasons> items, int page, int spaces) {
        ...
        for (int i = lowerBound; i < upperBound; i++) {
            try {
                reasons.add(new MuteReasons(items.get(i).getName(), items.get(i).getID(), items.get(i).getLength()));
            } catch(IndexOutOfBoundsException e) {
                continue;
            }
        }
